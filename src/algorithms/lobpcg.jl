## source: https://github.com/JuliaMolSim/DFTK.jl/blob/master/src/eigen/lobpcg_hyper_impl.jl
...
@timing function _LOBPCG(A, X, B=I, precon=I, tol=1e-10, maxiter=100;
    miniter=1, ortho_tol=2eps(real(eltype(X))),
    n_conv_check=nothing, display_progress=false, ignore_warnings=false)
    N, M = size(X)
...
Base.@kwdef struct LOBPCG{P,T,O}
    preconditioner::P = I
    tol::T = 1e-10
    ortho_tol::O = 2 * eps(Float64)
    maxiter::Int = 150
    miniter::Int = 1
